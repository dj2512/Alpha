...
public partial class _Default : System.Web.UI.Page
{
    ...
    protected void Assign_Click(object sender, EventArgs e)
    {
        ...
        if (a >= 8)
        {
            ScriptManager.RegisterStartupScript(Page, this.GetType(), "Key", string.Format("alert('{0}'); ","Maximum books issued i.e 8"),true);
        }
        else
        {
            a++;
            ...
            {
                ScriptManager.RegisterStartupScript(Page, this.GetType(), "Key", string.Format("alert('{0}'); ","Serial number doesn't exist or no copies are available" + ex),true);
