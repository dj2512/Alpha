...
public partial class depthead : System.Web.UI.MasterPage
{
    ...
    protected void clrissues_Click(object sender, EventArgs e)
    {
        try
        {
            using (SqlConnection sq = new SqlConnection(cs))
            {
                sq.Open();
                SqlCommand c = new SqlCommand("truncate table issues", sq);
                c.ExecuteNonQuery();
                sq.Close();
    ...
    protected void reset_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            {
                sq.Open();
                SqlCommand c = new SqlCommand("update  auth set location='NULL', cstatus='Free'", sq);
                c.ExecuteNonQuery();
                sq.Close();
